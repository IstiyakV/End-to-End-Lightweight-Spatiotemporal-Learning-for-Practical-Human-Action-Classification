...
def load_model(checkpoint_path: str, device: str = config.DEVICE) -> Tuple[torch.nn.Module, List[str]]:
    ...
    between legacy plain 3D CNN, Google Colab 3D CNN, advanced R(2+1)D Residual model checkpoints,
    and heavy torchvision video backbones (r3d_18, mc3_18, r2plus1d_18) or Hybrid transfer models.
    """
    ...
    # 1. Detect if it's a HybridTransferModel (contains keys starting with 'backbone.')
    is_hybrid = any(k.startswith("backbone.") for k in state_dict.keys())
    
    if is_hybrid:
        print("[COMPATIBILITY] Detected HybridTransferModel from Transfer Learning.")
        from .transfer_model import HybridTransferModel
        
        backbone_arch = ckpt["config"].get("backbone_architecture", "r3d_18")
        if backbone_arch == "r3d_18":
            backbone = video_models.r3d_18(weights=None)
            feat_dim = 512
        elif backbone_arch == "mc3_18":
            backbone = video_models.mc3_18(weights=None)
            feat_dim = 512
        elif backbone_arch == "r2plus1d_18":
        ...
        model = HybridTransferModel(backbone=backbone, custom_head=custom_head, backbone_feature_dim=feat_dim, freeze_backbone=False).to(device)
        model.is_colab = False
        
    else:
        # 2. Detect if it is a plain torchvision video model (contains layers starting with 'layer')
        is_torchvision = any(k.startswith("layer") for k in state_dict.keys())
        
        if is_torchvision:
            is_r2plus1d = any("stem.3.weight" in key for key in state_dict.keys())
            is_mc3 = any(".0.0.weight" in key for key in state_dict.keys()) and not is_r2plus1d
            
            if is_r2plus1d:
                print("[COMPATIBILITY] Instantiating pre-trained R(2+1)D-18 model.")
                model = video_models.r2plus1d_18(num_classes=num_classes).to(device)
            elif is_mc3:
                print("[COMPATIBILITY] Instantiating pre-trained MC3-18 model.")
                model = video_models.mc3_18(num_classes=num_classes).to(device)
            else:
                print("[COMPATIBILITY] Instantiating pre-trained ResNet3D-18 (r3d_18) model.")
                model = video_models.r3d_18(num_classes=num_classes).to(device)
            
            model.is_colab = False
            
        else:
            # 3. Detect standard simple 3D CNNs
            is_colab_checkpoint = any("conv1.1.running_mean" in key for key in state_dict.keys())
...
def predict_video(
    ...
    """
    ds = VideoDataset([video_path], [0], is_train=False, uniform_sample=True)
    frames, _ = ds[0]
...
def predict_realtime(
    ...
    stop_event = None,    # Threading event to stop the loop
    frame_callback = None # Callback to send frame and prediction back to GUI
):
    """
    ...
    latest_prediction = {"class": "Waiting for frames...", "confidence": 0.0, "latency_ms": 0.0}
    ...
    def inference_thread():
        ...
        while not stop_event.is_set():
            ...
            inference_latency = (time.perf_counter() - t0) * 1000.0  # ms
            ...
            latest_prediction = {
                ...
                "all_probs": {class_names[i]: float(smoothed_probs[i]) for i in range(len(class_names))}
            }
    ...
    try:
        ...
        while cap.isOpened() and not stop_event.is_set():
            ...
            fps = 1.0 / max(1e-5, current_time - last_frame_time)
            ...
            if frame_callback is not None:
                # Call callback to render in GUI (pass raw frame, prediction, and stats)
                frame_callback(
                    frame, 
                    latest_prediction['class'], 
                    latest_prediction['confidence'],
                    fps,
                    latest_prediction.get('latency_ms', 0.0),
                    latest_prediction.get('all_probs', {})
                )
                time.sleep(0.02)  # Cap frame rate at ~50 FPS
            else:
                # Draw Dynamic UI Overlay for external window
                overlay = frame.copy()
                ...
                try:
                    ...
                    if "not implemented" in str(e):
                        raise RuntimeError("Headless OpenCV detected. Real-time video window cannot be opened.\n"
                                           "Please run these commands in your terminal to fix this:\n"
