...
class GradCAM3D:
    ...
    def __init__(self, model: Compact3DCNN, target_layer: Optional[str] = None):
        self.model = model
        self.model.eval()

        # Default: hook last Conv3D in conv3 block
        if target_layer is None:
            self.target_layer = model.conv3[0]  # The Conv3d layer inside Sequential
        else:
    ...
    def __call__(
        ...
        img_size = getattr(self.model, "img_size", config.IMG_SIZE)
        cam = cam.unsqueeze(0).unsqueeze(0)  # (1, 1, T', H', W')
        cam = F.interpolate(
            cam,
            size=(config.N_FRAMES, *img_size),
            mode="trilinear",
            align_corners=False,
        )
...
def generate_gradcam_overlay(
    ...
    ds = VideoDataset([video_path], [0], is_train=False, img_size=img_size)
